...
class outputResults_xml():
	...
	'''
	container = '<results><testproject name="Fusion" prefix="FUS" />%s%s</results>'
	testplanNode = '<testplan name="%s" />'
	buildversionNode = '<build name="%s"/>'
	platformNode = '<platform name="%s"/>'
	resultnode = '<testcase external_id="%s"><result>%s</result><notes>%s</notes></testcase>'
	def __init__(self,sheetname, testplanname, versionname, platformname=None):
	...
	def create_resultxml(self, xlspath):
		...
		headerxml = self.testplanNode%self.testplanname + self.buildversionNode%self.versionname
		if self.platformname:
			headerxml = headerxml + self.platformNode%self.platformname
		
		resultxml = self.container%(headerxml, resultxml)
...
class outputResults_xls():
	...
	def run(self, fullpath, dstname=''):
		...
		for title in titles:
			...
			self.write_file(titles.index(title),0,names[0])#testlink id
			self.write_file(titles.index(title),1,names[1])
			self.write_file(titles.index(title),2,status)
		xmlfile.close()
		if not dstname:
...
if __name__=='__main__': 
	...
	if type == 'result':
		...
		sheetName = raw_input("Input the sheet name of sanityTestResult.xls:")
		if not sheetName:
			print 'not a valid sheet name!'
			sys.exit()
		testplanName = raw_input("Input the test plan name in TestLink:")	
		if not testplanName:
			print 'not a valid test plan name!'
			sys.exit()	
		platformName = raw_input("Input the platform name in TestLink(Leave it blank if you don't need it!):")
		buildVersion = raw_input("Input the build version in TestLink:")
		if not buildVersion:
			print 'not a valid build version!'
			sys.exit()	
		fullXlsPath = raw_input("Input the full path of sanityTestResult.xls:")
		if not fullXlsPath:
			print 'not a valid xls path!'
			sys.exit()
		if	platformName:	
			test = outputResults_xml(sheetName, testplanName, buildVersion, platformName)
		else:
			test = outputResults_xml(sheetName, testplanName, buildVersion)
